...
function QRDecomposition(A: IMatrix, epsilon: number): [IMatrix, IMatrix] {
	...
	let R = A.map(row => [...row]) // Deep copy of A
...
function matrixToString(matrix: IMatrix): string {
	return matrix.map(row => row.map(num => num.toFixed(4)).join('\t')).join('\n')
}
...
export function runQRAlgorithm(A: IMatrix, epsilon: number): string {
	let n = A.length
	...
	do {
		...
		res += `Iteration ${iter + 1}:\n`
		res += 'Matrix Q:\n' + matrixToString(Q) + '\n'
		res += 'Matrix R:\n' + matrixToString(R) + '\n'
		res += 'Updated Matrix A:\n' + matrixToString(A) + '\n'
		...
		for (let i = 0; i < n; i++) {
			if (
				...
				if (discriminant < 0) {
					...
					i++ // Skip next element as it's part of this 2x2 submatrix
				} else {
					// Real eigenvalues, similar to previous case
					current[i] = [A[i][i], 0]
				}
			}
		}
		...
		iter++
		// Here you can build up the `res` string with your results for each iteration
	} while (!flag)

	res += '\nEigenvalues:\n'
	for (const eigenvalue of prev) {
		res += complexToString(eigenvalue) + ' '
	}
	res += '\nTotal Iterations: ' + iter + '\n'
	// Include final results and eigenvalues in the `res` string
